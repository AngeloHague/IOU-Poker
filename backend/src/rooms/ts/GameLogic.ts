...
export function startGame(room: Room, state: GameState) {
  ...
  export function startRound(room: Room, state: GameState) {
    ...
    // assign first player
    state.player_idx = state.dealer_idx+1
...
// PLAYER (client) TURN CODE
 ...
 export function allocateWinnings(room: Room, state: GameState, winners: string[]) {
    winners.forEach((winner) => {
      try {
        if (state.pot > 0) {
          let max_payout = state.players.get(winner).current_bet * state.betting_players.size
          if (max_payout >= state.pot) {
            console.log(winner, ' has won ', state.pot,' chips')
            state.players.get(winner).chips += state.pot
            state.pot = 0
          } else {
            if (max_payout < state.pot) {
              console.log(winner, ' has won ', max_payout,' chips')
              state.players.get(winner).chips += max_payout
